...
void trya(string s, int n, int i, int j, string c[4][4], int dothing)
{
    ...
    for (int k = 0; k < 8; k++)
    {
        if ((i + dx[k] >= 0) && (i + dx[k] <= 3) && (j + dy[k] >= 0) && (j + dy[k] <= 3))
            if (visited[i + dx[k]][j + dy[k]] == false)
            {
...
int main()
{
    freopen("./thebogglegame/thebogglegame.inp", "r", stdin);
    freopen("./thebogglegame/thebogglegame.out", "w", stdout);
    while (cin >> a[0][0])
    {
        if (a[0][0] == "#")
            break;
