...
double minimalDistance(vector<point> &xPoints, vector<point> &yPoints, int left, int right)
{
    if (right - left <= 3)
        return bruteForce(xPoints, left, right);
    int mid = (right + left) / 2;
    point midPoint = xPoints[mid];
    double dist_left = minimalDistance(xPoints, yPoints, left, mid);
    double dist_right = minimalDistance(xPoints, yPoints, mid + 1, right);
    double dist_min = min(dist_left, dist_right);
    vector<point> strip;
    for (int i = left; i < right; i++)
        if (abs(yPoints[i].x - midPoint.x) < dist_min)
            strip.push_back(yPoints[i]);
    return min(dist_min, stripClosest(strip, dist_min));
}
int main()
{
    freopen("closestpairofpoints.inp", "r", stdin);
    freopen("closestpairofpoints.out", "w", stdout);
    int n;
    while (cin >> n)
    {
        if (n == 0)
        ...
        {   
            ...
            double result = minimalDistance(xPoints, yPoints, 0, n);
            if ((result >= 10000))
                cout << "INFINITY" << endl;
            else
            {
